...
ll dp[20][2][16][10][10][2];
string s;
ll func(ll digits, bool tight, ll xor_no, ll mn, ll mx, bool z, ll pos)
{
    if(digits==0)
        return (2*xor_no > mn+mx);
    if(dp[digits][tight][xor_no][mn][mx][z]!=-1)
        return dp[digits][tight][xor_no][mn][mx][z];
    ll end = 9;
    if(tight)
        end = s[pos]-'0';
    ll ans = 0;
    ...
    {
        if(i==0 and z)
            ans += func(digits-1, (tight && (i==end)), xor_no, mn, mx, 1, pos+1);
        else
            ans += func(digits-1, (tight && (i==end)), xor_no^i, min(mn, i), max(mx, i), 0, pos+1);
    }
    return dp[digits][tight][xor_no][mn][mx][z] = ans;
}
ll solve(ll n)
{
    if(n<=0)
...
of the digits is greater than average of maximum and minimum
digit in it

Z is zero here, and if it is 1, till now the number only contains
zeroes.

xor_no is xor till now
...
int main()
{
    ...
    testcase
    {
        ll a,b;
        cin>>a>>b;
        cout<<mod(solve(b)-solve(a-1))<<endl;
    }
    timetaken;
    return 0;
}
