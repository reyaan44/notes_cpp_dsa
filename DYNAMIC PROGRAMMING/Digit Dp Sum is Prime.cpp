...
ll dp[10][2][100];
string s;
ll func(ll n, ll tight, ll sum, ll pos)
{
    if(n==0)
        return isPrimeSieve[sum];
    if(dp[n][tight][sum]!=-1)
        return dp[n][tight][sum];
    dp[n][tight][sum] = 0;
    if(!tight)
    {
        for(ll i=0; i<=9; i++)
            dp[n][tight][sum] += func(n-1, 0, sum+i, pos+1);
    }
    else
    {
        ll curr = s[pos]-'0';
        for(ll i=0; i<curr; i++)
            dp[n][tight][sum] += func(n-1, 0, sum+i, pos+1);
        dp[n][tight][sum] += func(n-1, 1, sum+curr, pos+1);
    }
    return dp[n][tight][sum];
}
ll solve(ll n)
...
Find total digits in range [a,b] where sum of digits is prime.
*/
int main()
{
    quick;
