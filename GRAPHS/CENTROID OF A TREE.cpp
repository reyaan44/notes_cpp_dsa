...
ll dfs(ll a, vector<vector<ll>> & adj, vector<ll> & subtreeSize, ll par)
{
    ll a1 = 1;
    for(ll i=0; i<adj[a].size(); i++)
    {
        if(adj[a][i]!=par)
        {
            a1+=dfs(adj[a][i],adj,subtreeSize,a);
        }
    }
    return subtreeSize[a] = a1;
}
ll centroid(ll a, vector<vector<ll>> & adj, vector<ll> & subtreeSize, ll par)
{
    for(ll i=0; i<adj[a].size(); i++)
    {
        if(adj[a][i]!=par)
        {
            if(subtreeSize[adj[a][i]]>((adj.size()-1)/2)) return centroid(adj[a][i], adj, subtreeSize, a);
        }
    }
    return a;
}
int main()
{
    ...
    dfs(1, adj, subtreeSize, -1);   // FINDING THE SUBTREE SIZE OF EVERY NODE
    cout<<centroid(1,adj,subtreeSize,-1)<<endl; // ONE DFS STARTING FROM ROOT, IF THE SUBTREE SIE OF ALL ITS CHILD NODES INDIVIDUALLY <=N/2, THIS IS CENTROID
                                                // ELSE MOVE TO THE CHILD WHOSE SUBTREE SIZE >N/2, BECAUSE WE WILL FIND CENTROID IN THIS TREE.
    timetaken;
    return 0;
}