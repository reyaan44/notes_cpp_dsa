...
void dfs(vector<vector<ll>> &adj, ll a, vector<ll> &color, vector<ll> &parent, ll &cycle_start, ll &cycle_end)
{
    color[a] = 1;
    for(auto child: adj[a])
    {
        ...
        {
            parent[child] = a;
            dfs(adj, child, color, parent, cycle_start, cycle_end);
...
int main()
{
    ...
    for(ll i=1; i<=n; i++)
    {
        if(color[i]==0)
        {
            dfs(adj, i, color, parent, cycle_start, cycle_end);
    ...
    else
    {
        ...
        for(ll curr = cycle_end; curr!=cycle_start; curr = parent[curr])
        {
            cycle.pb(curr);
        }
        cycle.pb(cycle_start);
