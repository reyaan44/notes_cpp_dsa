...
int main()
{
    ...
    vector<vector<ll>> edge(n+1);   // STORING K SMALLEST PATH SUM FOR EVERY EDGE
    for(ll i=0; i<m; i++)
    {
        ll a,b,c;
        cin>>a>>b>>c;
    ...
    pq.push({0,1});
    while(!pq.empty() && edge[n].size()<k)
    {
        ll node = pq.top().ss;
        ll len = pq.top().ff;
        pq.pop();
        if(edge[node].size()>=k) continue;
        edge[node].pb(len);
        for(ll i=0; i<adj[node].size(); i++)
        {
            ll child = adj[node][i].ff;
            ll sz = adj[node][i].ss;
            if(edge[child].size()>=k) continue;
            pq.push({len + sz, child});
        }
    }
    printit(edge[n], k);
    timetaken;
